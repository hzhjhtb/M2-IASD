...
def in_plane(points, pt_plane, normal_plane, threshold_in=0.1):
    ...
    # The distance from point x to plane (p, n) is given by |(x-p) * n|
    distances = np.abs(np.dot(points - pt_plane, normal_plane))
    
    # Determine inliers as points where the distance to the plane is less than threshold_in
...
def recursive_RANSAC(points, nb_draws=100, threshold_in=0.1, nb_planes=2):
    ...
    for i in range(nb_planes):
        ...
        pt_plane, normal_plane, _ = RANSAC(points[remaining_inds], nb_draws, threshold_in)
        # Find inliers for the current plane
        inliers = in_plane(points[remaining_inds], pt_plane, normal_plane, threshold_in)
...
if __name__ == '__main__':
    ...
    if False:
        ...
        write_ply('../data/plane.ply', [points[plane_inds], colors[plane_inds], labels[plane_inds]], ['x', 'y', 'z', 'red', 'green', 'blue', 'label'])
        write_ply('../data/remaining_points_plane.ply', [points[remaining_inds], colors[remaining_inds], labels[remaining_inds]], ['x', 'y', 'z', 'red', 'green', 'blue', 'label'])
        ...
        write_ply('../data/best_plane.ply', [points[plane_inds], colors[plane_inds], labels[plane_inds]], ['x', 'y', 'z', 'red', 'green', 'blue', 'label'])
        write_ply('../data/remaining_points_best_plane.ply', [points[remaining_inds], colors[remaining_inds], labels[remaining_inds]], ['x', 'y', 'z', 'red', 'green', 'blue', 'label'])
        ...
        write_ply('../data/best_planes.ply', [points[plane_inds], colors[plane_inds], labels[plane_inds], plane_labels.astype(np.int32)], ['x', 'y', 'z', 'red', 'green', 'blue', 'label', 'plane_label'])
        write_ply('../data/remaining_points_best_planes.ply', [points[remaining_inds], colors[remaining_inds], labels[remaining_inds]], ['x', 'y', 'z', 'red', 'green', 'blue', 'label'])
    ...
    if True:
        ...
        # Save the best planes and remaining points
        write_ply('../data/best_planes_Lille.ply', [points[plane_inds], plane_labels.astype(np.int32)], ['x', 'y', 'z', 'plane_label'])
        write_ply('../data/remaining_points_best_planes_Lille.ply', [points[remaining_inds], ], ['x', 'y', 'z'])
